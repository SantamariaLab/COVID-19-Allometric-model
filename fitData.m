function out=fitData(totalt0,d2f,range2f,region,caseTh,dateV)
t=1:length(totalt0);
%allometric 
fa=fittype('((gamma/beta)*(1-exp((b-1)*beta*x))+x0.^(1-b)*exp((b-1)*beta*x))^(1/(1-b))','problem','x0');
ga=fitoptions(fa);
ga.Lower=[-2 0 0  ];
ga.Upper=[0.8 Inf Inf ];
...
ga.StartPoint=[-0.243 0.00045 diff(totalt0([1 5]))/5 ];
[fa0,gofa0]=fit(thisT2f(1:end)'-thisT2f(1),totalt0(thisT2f(1:end)'),...
    fa,ga,'problem',totalt0(thisT2f(1)));
